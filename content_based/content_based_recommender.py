...
def get_recommendations2(titles, cosine_sim):
    recommendation = dict()
    for title in titles:
        if title not in listGames:
            continue
        ...
        # Get the scores of the most similar games
        # (not the first one because this games as a score of 1 (perfect score) similarity with itself)
        sim_scores = sim_scores[1:3 * n_recommendation + 1]

        # Get the games indices
        game_indices = [i[0] for i in sim_scores]
        game_name = dataGames['name'].iloc[game_indices].tolist()
        count = 0
        for i in game_name:
            if i in titles:
                continue
            if i in recommendation:
                recommendation[i] += sim_scores[count][1]
            else:
                recommendation[i] = sim_scores[count][1]
            count += 1
    reco_scores = sorted(recommendation.items(), key=lambda x: x[1], reverse=True)[:n_recommendation]
    game_name_result = []
    for i in reco_scores:
        game_name_result.append(i[0])
...
def make_recommendation_for_user(user_id, game_list, game_user_have):
    ...
    if len(game_list) < n_recommendation:
        return DataFrame(data=[[user_id] + game_list +
                               [""] * (n_recommendation - len(game_list))],
                         columns=col_names)
    else:
        return DataFrame(data=[[user_id] + game_list[0:n_recommendation]],
                         columns=col_names)
    ...
    for j, row in dataUsers.iterrows():
        if previousId != row["user_id"]:
            if previousId == "":
                ...
                listSuggestion = get_recommendations2(listGamesUserHas, cosine_sim_matrix)
...
# generate_recommendation_output('Tags',
#                                pathlib.Path(
#                                    r'../../data/output_data/content_based_recommender_output_popular_tags.csv'))
# generate_recommendation_output('Genre',
#                                pathlib.Path(r'../../data/output_data/content_based_recommender_output_genre.csv'))
# generate_recommendation_output('genre_publisher_developer',
#                                pathlib.Path(
#                                    r'../../data/output_data/content_based_recommender_output_genre_publisher_developer.csv'))
# generate_recommendation_output('genre_tags_developer',
#                                pathlib.Path(
#                                    r'../../data/output_data/content_based_recommender_output_genre_popular_tags_developer.csv'))
# generate_recommendation_output('genre_tags_description',
#                                pathlib.Path(
#                                    r'../../data/output_data/content_based_recommender_output_genre_popular_tags_game_details.csv'))
# generate_recommendation_output('genre_publisher_developer_short_description',
#                                pathlib.Path(
#                                    r'../../data/output_data/content_based_recommender_output_genre_publisher_developer_game_details.csv'))
# generate_recommendation_output('all_out',
#                                pathlib.Path(r'../../data/output_data/all_out.csv'))
